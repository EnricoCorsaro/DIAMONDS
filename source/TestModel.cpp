...
int main(int argc, char *argv[])
{
    ...
    parametersMinima <<  -1.0, -1.0;         // Parameter 1, Parameter2
    ...
    */ 
    ...
    double initialEnlargementFactor = 1.0;  
    double alpha = 0.8;                         // Exponent for remaining prior mass in ellipsoid enlargement factor
    ...
    results.writeParametersToFile(outputDirName + "/test1_Parameter");
    results.writeLogLikelihoodToFile(outputDirName + "/test1_LogLikelihood.txt");
    results.writeEvidenceInformationToFile(outputDirName + "/test1_Evidence.txt");
    results.writePosteriorProbabilityToFile(outputDirName + "/test1_Posterior.txt");
