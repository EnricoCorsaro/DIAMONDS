...
// Source code file "QuadraticModel.cpp"

// To compile: clang++ -o BayesianInferenceQuadratic BayesianInferenceQuadratic.cpp -L../build/ -I ../include/ -l diamonds -stdlib=libc++ -std=c++11 -Wno-deprecated-register
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 1)
    {
        cerr << "Usage: ./BayesianInferenceQuadratic" << endl;
        exit(EXIT_FAILURE);
    }
    ...
    string inputFileName = "quadratic_data.txt";
    string outputPathPrefix = "BayesianInferenceQuadratic_";
    ...
    parametersMinima <<  0.01, 0.001, 2.0;         // Minima values for the free parameters (free parameter #1, free parameter #2, ..., etc.)
    parametersMaxima << 2.0, 0.2, 40;              // Maxima values for the free parameters (same order as minima)
    ...
    QuadraticModel model(covariates);      // Quadratic function of the type f = a*x + b*x^2 + c
